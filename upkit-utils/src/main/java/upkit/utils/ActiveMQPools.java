...
public class ActiveMQPools {
	...
	/**
	 * 指定Session每隔多长时间就要提交一次，单位是毫秒，主要就是为了防止消息数量少了， 达不到上面一个参数指定的阀值
	 */
	...
	/**
	 * 缓存到目的地的连接对象，key-第几个连接对象，也就是连接对象的编号 value-对应的连接对象
	 */
	...
	public void init() {
		...
		((ActiveMQConnectionFactory) pooledConnectionFactory.getConnectionFactory())
				.setProducerWindowSize(Integer.MAX_VALUE);
	...
	/**
	 * 提交Session，如果Session中需要发送的消息数量达到了要提交的阀值，就提交； 如果没有达到，就把对应的需要发送的消息计数加1
	 * 
	 * @param session
	 */
	...
	/**
	 * 按照时间去检查，并提交Session中的消息的线程 在第一个调用sessionCommit方法的时候启动
	 * 在检测到所有的session都没有需要提交的消息后，自动终止 然后再次在有人调用sessionCommit方法的时候启动
	 */
		...
		public void run() {
			...
			while (flag) {
				...
				for (Session s : set) {
					...
					if (mapSessionLastCommitTime.get(s) == null || (System.currentTimeMillis()
							- mapSessionLastCommitTime.get(s)) >= sessionCommitInterval) {
						// 该提交了
						try {
