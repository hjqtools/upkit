...
import upkit.utils.bean.AnnotationDescMaker.AnnotationWrapper;
...
public class InvokerCreateor implements Opcodes {
	...
	private static final Logger logger = LoggerFactory.getLogger(InvokerCreateor.class);
	...
	public static byte[] generatorInstanceClass(String invokerClassName, Class<?> hostClass) {
		...
		String className = (StringUtils.isBlank(invokerClassName) ? hostClass.getName() + "$Instance"
				: invokerClassName);
	...
	public static byte[] generatorClass(String invokerClassName, Class<?> hostClass, Method method) {
		String hostClassDesc = Type.getDescriptor(hostClass);
		String hostClassInnerName = Type.getInternalName(hostClass);
		String className = (StringUtils.isBlank(invokerClassName) ? hostClass.getName() + "$" + method.getName()
				+ "$" + Math.abs(Type.getMethodDescriptor(method).hashCode()) : invokerClassName);
		className = className.replace(".", "/");

		Class<?> returnType = method.getReturnType();
		logger.debug("retType : {} " + returnType.getName());
		boolean primitive = returnType.isPrimitive();
		boolean hasRet = !(returnType == void.class);
		logger.debug("has ret : " + hasRet);
	...
	private static void visitWithNotRet(MethodVisitor mv, Class<?>[] argumentTypes, Method method, String className,
		...
		if (hasArguments) {
			...
			for (Class<?> arguType : argumentTypes) {
				logger.debug("arguType:" + arguType.getName() + "i : " + i);
		...
		logger.debug("baseIndex : " + baseIndex);
		...
		mv.visitLabel(l2);
		if (hasArguments) {
			logger.debug("maxStack: " + (1 + argumentTypes.length) + " , maxLocal" + (3 + argumentTypes.length));
	...
	private static void visitWithHasRet(MethodVisitor mv, Class<?>[] argumentTypes, Method method, String className,
		...
		int oldBaseIndex = 2;
		int baseIndex = 2;
		logger.debug("arguType are : " + argumentTypes.length);
		if (hasArguments) {
			int i = 0;
			for (Class<?> arguType : argumentTypes) {
				logger.debug("arguType:" + arguType.getName() + "i : " + i);
		...
		logger.debug("baseIndex is : " + baseIndex);
		Label l0 = new Label();
		mv.visitLabel(l0);
		mv.visitVarInsn(ALOAD, 1);
		int i = oldBaseIndex + 1;
	...
	private static Method getPrimitiveMethod(Class<?> wrapperType) {
		...
		logger.debug("wrapperType : " + wrapperType + " and methodName is : " + methodName);
		try {
	...
	public static void primitiveChange(MethodVisitor mv, Class<?> primitiveType) {
		...
		logger.debug(Type.getInternalName(wrapperClass) + "----" + Type.getMethodDescriptor(wrapperTypeMethod));
