...
public class BaseDistributedLock {
	...
	private boolean waitToLock(long startMillis, Long millisToWait, String ourPath) throws Exception {
		...
		try {
			while (!haveTheLock) {
				// 该方法实现获取lock节点下的所有顺序节点，并且从小到大排序
				List<String> children = getSortedChildren();
				String sequenceNodeName = ourPath.substring(basePath.length() + 1);

				// 计算刚才客户端创建的顺序节点在locker的所有子节点中排序位
				// 置，如果是排序为0，则表示获取到了锁
				...
				// 如果当前客户端创建的节点在locker子节点列表中位置大于0，则表示其他客户端已经获取了锁
				// 此时当前客户端需要等待其他客户端锁释放
				boolean isGetTheLock = ourIndex == 0;

				// 如何判断其它客户端是否已经释放了锁？从子节点列表中获取到比自己
				// 次小的那个节点，并对其建立监听
				...
				if (isGetTheLock) {
					...
					/*
					 * 如果次小的节点被删除了，则表示当前客户端的节点应该是最小的了，所以使 用CountDownLatch来实现等待
					 */
	...
	protected String attemptLock(long time, TimeUnit unit) throws Exception {
		...
		while (!isDone) {
			isDone = true;
			try {
				// createLockNode 用于在locker(basePath持久节点) 下创建客户端要获取锁的[临时]顺序节点
